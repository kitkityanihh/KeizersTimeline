...
function easyScrollDots(dotfixedOptions) {
  ...
  dotfixedOptions.fixedNav === true
    ? (dotFixedNavPresent = true)
    : dotFixedNavPresent;
  dotfixedOptions.fixedNavId === ""
    ? (dotFixedNavId = false)
    : (dotFixedNavId = dotfixedOptions.fixedNavId);
  dotfixedOptions.fixedNavUpward === true
    ? (dotFixedNavUp = true)
    : dotFixedNavUp;
  ...
  if (scrollIndi.length) {
    ...
    document
      .querySelector("body")
      .lastElementChild.insertAdjacentHTML("afterend", scrollIndiTemplate);
    const scrollIndiController = document.querySelector(
      ".scroll-indicator-controller"
    );
    if (
    ...
    const scrollIndiControllerDots = scrollIndiController.querySelectorAll(
      "[data-indi-controller-id]"
    );
    ...
    var handleIndiScroll = dotsThrottle(function () {
      ...
      const indiOffsetValues = Object.keys(indiScrollTopCollection).map(
        function (itm) {
